...
const notify = (mess,type) =>{ 
    ...
    async function handelRegesiter (values){
        //Call APi
        setisloading(true);
        let {data}=await axios.post(`https://route-ecommerce-app.vercel.app/api/v1/auth/signup` ,values).then((data)=>{
    ...
    <div className="w-75 mx-auto py-4">
        ...
        <form onSubmit={formik.handleSubmit}>
            ...
            <input onBlur={formik.handleBlur}  onChange={formik.handleChange}  type="tel" className='form-control mb-2' name='phone' id='phone' value={formik.values.phone}/>
            {formik.errors.phone && formik.touched.phone?<div className="alert alert-danger">{formik.errors.phone}</div>:null}
            
            {isloading? <button className='btn bg-main text-white'><i className='fa fa-spinner fa-spin'></i></button>:            <button type='submit' disabled={!(formik.isValid &&formik.dirty)} className='btn bg-main text-white'>Register</button>
}
